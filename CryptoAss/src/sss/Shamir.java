...
public final class Shamir
{
    public static SecretShare[] split(final BigInteger secret, int needed, int available, BigInteger prime, Random random)
    {
        ...
        coeff[0] = secret;
        //coeff[1] = BigInteger.valueOf(10);// random
        //coeff[2] = BigInteger.valueOf(2);// random
    ...
    public static BigInteger combine(final SecretShare[] shares, final BigInteger prime)
    {
        ...
        for(int formula = 0; formula < shares.length; formula++)
        {
            ...
            System.out.println("modInverse: " + modInverse(denominator, prime));
    ...
    public static void main(final String[] args)
    {
        ...
        final BigInteger secret = new BigInteger("3"); //123

        // prime number must be longer then secret number
        final BigInteger prime = new BigInteger("17"); //new BigInteger(secret.bitLength() + 1, CERTAINTY, random);

        // 2 - at least 2 secret parts are needed to view secret
        // 5 - there are 5 persons that get secret parts
        final SecretShare[] shares = Shamir.split(secret, 2, 6, prime, random);


        
        SecretShare[] sharesToViewSecret = new SecretShare[] {shares[0],shares[2]}; // 0 & 1
